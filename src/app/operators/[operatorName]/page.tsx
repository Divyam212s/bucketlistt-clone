...
type Props = { params: Promise<{ operatorName: string }> };
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    const { operatorName } = await params;
    const title = slugToTitle(operatorName);
    return {
        title: `${title} | Bucketlistt Operators`,
        description: `Book activities with ${title}. Curated adventure experiences.`,
    };
}

export default async function OperatorPage({ params }: Props) {
    const { operatorName } = await params;
    const operatorTitle = slugToTitle(operatorName);
    ...
    return (
        <main>
            ...
            <ActivityGallery
                ...
                destinationHref="/all-destination/rishikesh"
                rating={4.9}
                reviewCount={1283}
                location="India"
                ...
                secondaryImageUrls={[
                    ...
                    "https://images.unsplash.com/photo-1605649487212-47bdab064df7?w=400&auto=format&fit=crop",
                ]}
            />
            <ActivityContent
